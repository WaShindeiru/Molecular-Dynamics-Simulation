...
use crate::simulations::examples::{
  dense_particles, one_particle_edge, sphere_particles, symmetric_triangle_test, triangle,
  two_particles_edge,
};
...
pub fn dense_runner() {
  let save_path = get_save_path("/media/washindeiru/7E442D59442D1585/md/".to_string());
...
pub fn one_particle_edge_runner() {
  ...
  let desired_temperatures = vec![TemperatureInfo::new(
    600.,
    TimeIterationDistance::Iteration { value: 1000 },
  )]
  .into_iter()
  .map(|i| TemperatureInfo {
    desired_temperature: i.desired_temperature / TEMPERATURE_U,
