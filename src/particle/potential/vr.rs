...
pub fn vr(r_mag: f64, interaction_type: &InteractionType) -> f64 {
...
pub fn vr_gradient(r_ij_vec: &Vector3<f64>, interaction_type: &InteractionType) -> Vector3<f64> {
    let Beta = get_constant(interaction_type, Constant::Beta);
    let S = get_constant(interaction_type, Constant::S);

    let r_ij_mag = r_ij_vec.magnitude();
    let common = vr(r_ij_mag, interaction_type) * (-Beta * (2. * S).sqrt());
