...
pub fn check_position_constraint_simple(
  ...
  (position, ParticleCompliance { compliant, x, y, z })
}

pub fn apply_velocity_constraint_simple(
  compliance: &ParticleCompliance,
  mut velocity: Vector3<f64>,
) -> Vector3<f64> {
  velocity.x = match compliance.x {
    Compliant => velocity.x,
    Compliance::ExceededLowerBoundary | Compliance::ExceededHigherBoundary => -velocity.x,
  };

  velocity.y = match compliance.y {
    Compliant => velocity.y,
    Compliance::ExceededLowerBoundary | Compliance::ExceededHigherBoundary => -velocity.y,
  };
