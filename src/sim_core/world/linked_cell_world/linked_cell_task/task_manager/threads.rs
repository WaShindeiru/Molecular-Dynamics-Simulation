...
use crate::sim_core::world::linked_cell_world::linked_cell_task::handle_task::{
...
fn worker_task_handle(
  job_rx: Arc<Mutex<Receiver<LinkedCellTask>>>,
  result_tx: Sender<LinkedCellResult>,
  num_atoms: usize,
) {
  ...
  loop {
    ...
    match job {
      Ok(task) => match task {
        LinkedCellTask::VelocityBatchTask {
          task_id,
          cell_ids,
          history,
          time_step,
          ...
          result_tx.send(LinkedCellResult::VelocityResult(velocity_result)).unwrap();
        }

        LinkedCellTask::ForceBatchTask { task_id, cell_ids, integration_cache } => {
          ...
          result_tx.send(LinkedCellResult::ForceResult(force_result)).unwrap();
        }
      },
      Err(_) => break,
    }
...
pub fn create_threads(
  ...
  Sender<LinkedCellTask>,
  Receiver<LinkedCellResult>,
  Vec<JoinHandle<()>>,
  usize,
) {
  ...
  let (tx_task, rx_task) = mpsc::channel::<LinkedCellTask>();
  let job_rx = Arc::new(Mutex::new(rx_task));
  let (result_tx, result_rx) = mpsc::channel::<LinkedCellResult>();
