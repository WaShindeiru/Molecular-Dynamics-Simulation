...
pub struct LinkedCellPersistanceReset {
  history_manager: LinkedCellHistoryManager,
...
impl LinkedCellPersistanceReset {
  pub fn new(
    history_manager: LinkedCellHistoryManager,
    config: SimulationConfig,
    num_of_atoms: usize,
  ...
  pub fn history_manager(&self) -> &LinkedCellHistoryManager {
    &self.history_manager
  }

  pub fn history_manager_mut(&mut self) -> &mut LinkedCellHistoryManager {
  ...
  pub fn begin_update_step(&mut self, num_of_world_iterations: usize) -> io::Result<()> {
    ...
    if self.reset_counter == self.config.max_iteration_till_reset {
      ...
      info!(
        "Resetting linked cell world, reset number: {}",
        self.number_of_resets
      );
...
impl Drop for LinkedCellPersistanceReset {
  fn drop(&mut self) {
