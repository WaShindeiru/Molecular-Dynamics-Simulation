...
const TEMPERATURE_CELCIUS: f64 = 100000.0;
const Q_EFFECTIVE_MASS: f64 = 5.0;


fn main() {
  two_particles_runner();
}
...
fn sphere_runner() {
  ...
  let save = true;
  let verbose = true;
  sphere_particles_thermostat(save, num_iterations, 40, temperature_kelvin_unitless,
                              Q_EFFECTIVE_MASS, verbose);
...
fn sphere_particles_thermostat(save: bool, num_iterations: usize, num_particles: usize,
                               temperature_kelvin: f64, q_effective_mass: f64, verbose: bool) {
  ...
  engine.run(save, false, temperature_kelvin, q_effective_mass, verbose);
}
